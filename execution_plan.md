...
- **Exit criterion:** a `Tenant` can be provisioned end-to-end (created, admin invited via `UserInvitation`, invitation accepted); a dummy entity can then be created, is correctly tenant-scoped, policy-gated, audit-logged, rejected if dated into a locked `AccountingPeriod`, and MFA-gated for Finance/Admin roles. **Tenant-isolation tests are part of this exit criterion, not a later addition:** an authenticated request from Tenant A's user attempting to read or write Tenant B's data must fail, tested explicitly, not just assumed to fail because the global scope exists.
