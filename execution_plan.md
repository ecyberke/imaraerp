...
- **Exit criterion:** a `Tenant` can be provisioned end-to-end (created, admin invited via `UserInvitation`, invitation accepted); a dummy entity can then be created, is correctly tenant-scoped, policy-gated, audit-logged, and MFA-gated for Finance/Admin roles. **`AccountingPeriod` behavior, corrected here to match §3.10 (the architecture doc is the source of truth — this plan's wording was wrong, not the implementation):** a posting dated into a `closed` period is never rejected outright — it auto-forwards to the earliest currently-*open* period (with the original intended date preserved on `JournalEntry.original_intended_posting_date`), unless the intended date itself falls within some other period that's also currently open, in which case it posts there directly rather than forwarding past it. The exit criterion is: a backdated post into a closed period demonstrably auto-forwards and preserves the original date — not that it's rejected. **Tenant-isolation tests are part of this exit criterion, not a later addition — and specifically an HTTP-level test, not a query-level one:** a model/repository test proving the Eloquent global scope adds a `WHERE tenant_id` clause is necessary but not sufficient. The test that actually satisfies this criterion authenticates as a real `User` belonging to Tenant A and issues a real HTTP request for a record belonging to Tenant B by ID, through the full routing → middleware → Policy → query stack — asserting a **404**, not a 403 (a 403 would itself leak that the record exists). If the isolation test written for this branch only exercises the scope at the model layer, it hasn't met this criterion yet.
...
- **Exit criterion:** the full ported `LedgerPostingServiceTest.php` suite passes against the real database.
- **Correction (this revision):** the four row-locking concurrency tests previously listed here — credit-limit check, Variation Order/BOQ-line mutation, Milestone sign-off, stock reservation — never belonged to this branch's own exit criterion in the first place: `ledger-core` (Phase 0) builds none of the entities those locks protect (`Party`, `VariationOrder`/`BOQLine`, `Milestone`, `StockLedger` reservation all belong to later branches), so "two simultaneous requests against the same record" had no real record to run against here. Each test now lives in the exit criterion of the branch that actually builds its entity — credit-limit check in `finance-billing` (already stated there), stock reservation in `inventory-core` (already stated there), and Variation Order/BOQ-line mutation + Milestone sign-off newly stated in `projects-milestones-ui` below. The general rule stays in the Cross-Cutting Reminders section: every branch introducing a concurrent-access risk tests it with an actual concurrent-request simulation, not just asserted in a comment.
...
- **Concurrency exit criterion (moved here from `ledger-core`, which named these four row-locking tests before it had any of the entities to run them against):** two simultaneous Variation Order approvals against the same `BOQLine` resolve to exactly one success and one clear retry/failure — the row-locked BOQ-line mutation named above, actually proven, not just implemented; two simultaneous Milestone sign-offs against the same `Milestone` resolve the same way. Both are new tests against this branch's own entities — real concurrent-request simulations, not asserted in a comment (Cross-Cutting Reminders' general rule, made concrete here).
...
- **Server-side quantity/rate validation (§1.1) has nothing to attach to in `platform-foundation` — correctly deferred there, but not a one-time task.** Every later branch that introduces a quantity or rate/amount field (`SalesOrderLine`, `PurchaseOrderLine`, `PurchaseRequisitionLine`, `BOQLine`, `MeasurementSheet`, `Timesheet` hours, and others) is individually responsible for enforcing it on that field the moment the field exists — zero/negative quantities rejected except `variation_type = omission`, negative rates/amounts rejected outright. This is a checklist item for each of those branches' own PRs, not a single deliverable that gets marked done once and forgotten.
